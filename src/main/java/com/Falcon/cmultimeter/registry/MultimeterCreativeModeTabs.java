...
public class MultimeterCreativeModeTabs {
	private static final DeferredRegister<CreativeModeTab> REGISTER =
			DeferredRegister.create(Registries.CREATIVE_MODE_TAB, MultimeterCreate.MOD_ID);
	...
	private static class RegistrateDisplayItemsGenerator implements CreativeModeTab.DisplayItemsGenerator {
		...
		private List<Item> collectBlocks(Predicate<Item> exclusionPredicate) {
			...
			for (RegistryEntry<Block> entry : MultimeterCreate.REGISTRATE.getAll(Registries.BLOCK)) {
		...
		private List<Item> collectItems(Predicate<Item> exclusionPredicate) {
			List<Item> items = new ReferenceArrayList<>();
			for (RegistryEntry<Item> entry : MultimeterCreate.REGISTRATE.getAll(Registries.ITEM)) {
				if (!CreateRegistrate.isInCreativeTab(entry, tabFilter))
					continue;
				Item item = entry.get();
